...
def netmf(ctrl, graph):
    ...
    return NetMF_Original(my_graph=graph, rank=args.rank, dim=ctrl.embed_dim, window=args.window_size, negative=args.negative,
                          large=args.large, small=args.small, logger=ctrl.logger, seed=ctrl.seed).get_embeddings()
...
class NetMF_Original(object):
    ...
    def __init__(self, my_graph, rank, dim, window=10, negative=1.0, large=True, small=False, logger=None, seed=20):
        self.logger = logger
        np.random.seed(seed)
